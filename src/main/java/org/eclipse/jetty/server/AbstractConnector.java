...
/**
 * <p>An abstract implementation of {@link org.eclipse.jetty.server.Connector} that provides a {@link org.eclipse.jetty.server.ConnectionFactory} mechanism
 ...
 * <ul>
 * <li>The {@link java.util.concurrent.Executor} service is used to run all active tasks needed by this connector such as accepting connections
 * or handle HTTP requests. The default is to use the {@link org.eclipse.jetty.server.Server#getThreadPool()} as an executor.
 * </li>
 ...
 * {@link java.nio.ByteBuffer} instances from a pool.  The default is to use a new {@link ArrayByteBufferPool} instance.
 * </li>
 * </ul>
 ...
 * The connector keeps a collection of {@link org.eclipse.jetty.server.ConnectionFactory} instances, each of which are known by their
 ...
 * an {@link org.eclipse.jetty.server.SslConnectionFactory} that has been instantiated with the {@link org.eclipse.jetty.server.HttpConnectionFactory} as it's
 * next protocol.
 *
 * <h4>Configuring Connection Factories</h4>
 * The collection of available {@link org.eclipse.jetty.server.ConnectionFactory} may be constructor injected or modified with the
 * methods {@link #addConnectionFactory(org.eclipse.jetty.server.ConnectionFactory)}, {@link #removeConnectionFactory(String)} and
 * {@link #setConnectionFactories(java.util.Collection)}.  Only a single {@link org.eclipse.jetty.server.ConnectionFactory} instance may be configured
 * per protocol name, so if two factories with the same {@link org.eclipse.jetty.server.ConnectionFactory#getProtocol()} are set, then
 ...
 * configure the HTTP protocol, you pass a {@link org.eclipse.jetty.server.HttpConfiguration} instance to the {@link org.eclipse.jetty.server.HttpConnectionFactory}
 * (or the SPDY factories that can also provide HTTP Semantics).  Similarly the {@link org.eclipse.jetty.server.SslConnectionFactory} is
 * configured by passing it a {@link SslContextFactory} and a next protocol name.
 *
 * <h4>Connection Factory Operation</h4>
 * {@link org.eclipse.jetty.server.ConnectionFactory}s may simply create a {@link Connection} instance to support a specific
 * protocol.  For example, the {@link org.eclipse.jetty.server.HttpConnectionFactory} will create a {@link org.eclipse.jetty.server.HttpConnection} instance
 * that can handle http/1.1, http/1.0 and http/0.9.
 * <p>
 * {@link org.eclipse.jetty.server.ConnectionFactory}s may also create a chain of {@link Connection} instances, using other {@link org.eclipse.jetty.server.ConnectionFactory} instances.
 * For example, the {@link org.eclipse.jetty.server.SslConnectionFactory} is configured with a next protocol name, so that once it has accepted
 * a connection and created an {@link SslConnection}, it then used the next {@link org.eclipse.jetty.server.ConnectionFactory} from the
 ...
 * {@link org.eclipse.jetty.server.SslConnectionFactory} will have a protocol name of "SSL-http/1.1" and lookup "http/1.1" for the protocol
 * to run over the SSL connection.
 * <p>
 * {@link org.eclipse.jetty.server.ConnectionFactory}s may also create temporary {@link Connection} instances that will exchange bytes
 ...
 * negotiate the version of SPDY or HTTP that the client and org.eclipse.jetty.server will speak.  Thus to accept a SPDY connection, the
 * connector will be configured with {@link org.eclipse.jetty.server.ConnectionFactory}s for "SSL-NPN", "NPN", "spdy/3", "spdy/2", "http/1.1"
 ...
 * <li>call the {@link #getDefaultConnectionFactory()} {@link org.eclipse.jetty.server.ConnectionFactory#newConnection(org.eclipse.jetty.server.Connector, org.eclipse.jetty.io.EndPoint)}
...
public abstract class AbstractConnector extends ContainerLifeCycle implements Connector, Dumpable
{
    ...
    // Order is important on org.eclipse.jetty.server side, so we use a LinkedHashMap
    ...
    /**
     * @param server The org.eclipse.jetty.server this connector will be added to. Must not be null.
     * @param executor An executor for this connector or null to use the servers executor
     * @param scheduler A scheduler for this connector or null to either a {@link Scheduler} set as a org.eclipse.jetty.server bean or if none set, then a new {@link ScheduledExecutorScheduler} instance.
     * @param pool A buffer pool for this connector or null to either a {@link ByteBufferPool} set as a org.eclipse.jetty.server bean or none set, the new  {@link ArrayByteBufferPool} instance.
            ...
            ConnectionFactory... factories)
    {
        ...
        addBean(_executor);
        if (executor==null)
            unmanage(_executor); // inherited from org.eclipse.jetty.server
        addBean(_scheduler);
    ...
    /**
     * <p>Sets the maximum Idle time for a connection, which roughly translates to the {@link java.net.Socket#setSoTimeout(int)}
    ...
    private class Acceptor implements Runnable
    {
        ...
        public void run()
        {
            ...
            current.setName(name + "-acceptor-" + _acceptor + "-" + org.eclipse.jetty.server.AbstractConnector.this);

            synchronized (org.eclipse.jetty.server.AbstractConnector.this)
            {
