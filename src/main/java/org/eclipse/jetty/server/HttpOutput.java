...
/**
 * <p>{@link org.eclipse.jetty.server.HttpOutput} implements {@link ServletOutputStream}
 * as required by the Servlet specification.</p>
 * <p>{@link org.eclipse.jetty.server.HttpOutput} buffers content written by the application until a
 * further write will overflow the buffer, at which point it triggers a commit
 * of the response.</p>
 * <p>{@link org.eclipse.jetty.server.HttpOutput} can be closed and reopened, to allow requests included
...
{
    private static Logger LOG = Log.getLogger(org.eclipse.jetty.server.HttpOutput.class);
    ...
    */
    ...
    private abstract class AsyncICB extends IteratingCallback
    {
        ...
        protected void completed()
        {
            try
            {
                while(true)
                {
                    org.eclipse.jetty.server.HttpOutput.OutputState last=_state.get();
                    switch(last)
                    {
                        case PENDING:
                            if (!_state.compareAndSet(org.eclipse.jetty.server.HttpOutput.OutputState.PENDING, org.eclipse.jetty.server.HttpOutput.OutputState.ASYNC))
                                continue;
                            break;

                        case UNREADY:
                            if (!_state.compareAndSet(org.eclipse.jetty.server.HttpOutput.OutputState.UNREADY, org.eclipse.jetty.server.HttpOutput.OutputState.READY))
                                continue;
                            _channel.getState().onWritePossible();
                            break;
     ...
     * A non direct buffer of size {@link org.eclipse.jetty.server.HttpOutput#getBufferSize()} is used.
     * This callback is passed to the {@link org.eclipse.jetty.server.HttpChannel#write(java.nio.ByteBuffer, boolean, Callback)} to
     ...
     * A {@link java.nio.ByteBuffer} of size {@link org.eclipse.jetty.server.HttpOutput#getBufferSize()} is used that will be direct if
