 ...
 * <p>An instance of this class will monitor all the connectors of a org.eclipse.jetty.server (or a set of connectors
 * configured with {@link #setMonitoredConnectors(java.util.Collection)}) for a low resources state.
...
{
    private static final Logger LOG = Log.getLogger(org.eclipse.jetty.server.LowResourceMonitor.class);
    ...
    public LowResourceMonitor(@Name("org/eclipse/jetty/server") Server server)
    {
        _server=server;
    }
    ...
    @ManagedAttribute("The monitored connectors. If null then all org.eclipse.jetty.server connectors are monitored")
