...
{
    public static final String UPGRADE_CONNECTION_ATTRIBUTE = "org.eclipse.jetty.org.eclipse.jetty.server.HttpConnection.UPGRADE";
    ...
    private static final Logger LOG = Log.getLogger(org.eclipse.jetty.server.HttpConnection.class);
    private static final ThreadLocal<org.eclipse.jetty.server.HttpConnection> __currentConnection = new ThreadLocal<>();
    ...
    protected static org.eclipse.jetty.server.HttpConnection setCurrentConnection(org.eclipse.jetty.server.HttpConnection connection)
    {
        org.eclipse.jetty.server.HttpConnection last=__currentConnection.get();
        if (connection==null)
    ...
    public void onFillable()
    {
        ...
        final org.eclipse.jetty.server.HttpConnection last=setCurrentConnection(this);
            ...
            switch (version)
            {
                ...
                case RTSP_1_0:
                {
                    ...
                    if (!persistent)
                        persistent = HttpMethod.CONNECT.is(getRequest().getMethod());
                    if (persistent)
                        //getResponse().getHttpFields().add(HttpHeader.CONNECTION, HttpHeaderValue.CLOSE); //todo check
                    break;
                }
