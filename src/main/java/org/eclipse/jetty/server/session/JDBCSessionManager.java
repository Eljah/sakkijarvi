...
{
    private static final Logger LOG = Log.getLogger(org.eclipse.jetty.server.session.JDBCSessionManager.class);

    private ConcurrentHashMap<String, Session> _sessions;
    protected org.eclipse.jetty.server.session.JDBCSessionIdManager _jdbcSessionIdMgr = null;
    ...
    public class Session extends AbstractSession
    {
        ...
        {
            super(org.eclipse.jetty.server.session.JDBCSessionManager.this,request);
            int maxInterval=getMaxInactiveInterval();
            _expiryTime = (maxInterval <= 0 ? 0 : (System.currentTimeMillis() + maxInterval*1000L));
            _virtualHost = org.eclipse.jetty.server.session.JDBCSessionManager.getVirtualHost(_context);
        ...
        {
            super(org.eclipse.jetty.server.session.JDBCSessionManager.this, created, accessed, sessionId);
            _rowId = rowId;
     ...
     *
     * @see org.eclipse.jetty.server.session.AbstractSessionManager#getSession(String)
     */
    @Override
    ...
    public void doStart() throws Exception
    {
        ...
        _jdbcSessionIdMgr = (org.eclipse.jetty.server.session.JDBCSessionIdManager)_sessionIdManager;
    ...
    /**
     *
     * @see org.eclipse.jetty.server.SessionManager#renewSessionId(String, String, String, String)
     */
     ...
     * the database.
     *
     * @see org.eclipse.jetty.server.session.AbstractSessionManager#removeSession(String)
     */
    @Override
    ...
    protected void addSession(AbstractSession session)
    {
        ...
        try
        {
            synchronized (session)
            {
                ...
                storeSession(((org.eclipse.jetty.server.session.JDBCSessionManager.Session)session));
                session.didActivate();
            }
        }
        catch (Exception e)
     ...
     * {@link org.eclipse.jetty.server.SessionIdManager#invalidateAll(String)} should be called.
     */
    @Override
            ...
            public void run()
            {
                        ...
                        ResultSet result = statement.executeQuery())
                {
                    ...
                    if (result.next())
                    {
                        ...
                        if (maxInterval == org.eclipse.jetty.server.session.JDBCSessionIdManager.MAX_INTERVAL_NOT_SET)
                        {
                        ...
                        try (InputStream is = ((org.eclipse.jetty.server.session.JDBCSessionIdManager)getSessionIdManager())._dbAdaptor.getBlobInputStream(result, _sessionTableSchema.getMapColumn());
