...
public class RTPSession {
	 ...
	 //protected int payloadType = 0; //TODO remove 26 as default!!
     //protected int payloadType = 96; //TODO remove 26 as default!!

     protected int payloadType = 26;
	...
	private void generateSsrc() {
		...
		// Set an SSRC
		//TODO this.ssrc = this.random.nextInt();
          this.ssrc=11;
		if(this.ssrc < 0) {
    ...
    /*
    public void  sendRtcpPktSR()
    {
        RtcpPktSR rtcppktsr=new RtcpPktSR()

    }
     */


    public void sendSenderReportPacket() {
        final int FIXED_HEADER_SIZE = 4;

        boolean rawPktCurrent = false;
        ...
        byte V_P_RC = (byte)((version << 6) | (padding << 5) | (0x00));

        byte ss[] = {(byte)(ssrc >> 24), (byte)(ssrc >> 16), (byte)(ssrc >> 8), (byte)(ssrc & 0xFF)};
        byte PT[] = {(byte)(_PT & 0xFF)};
        Date date = new Date();
        long time=date.getTime();

        byte NTP_TimeStamp[] = {(byte)(time >> 56),(byte)(time >> 48), (byte)(time >> 40), (byte)(time >> 32),(byte)(time >> 24), (byte)(time >> 16), (byte)(time >> 8), (byte)(time & 0xFF)};
        //short randomOffset = (short)Math.abs(rand.nextInt() & 0x000000FF);
        byte RTP_TimeStamp[] = {(byte)(time >> 24), (byte)(time >> 16), (byte)(time >> 8), (byte)(time & 0xFF)};
        byte SenderPacketCount[] = {(byte)(time >> 24), (byte)(time >> 16), (byte)(time >> 8), (byte)(time & 0xFF)}; //todo no matter what it is?
        byte SenderOctetCount[] = {(byte)(time >> 24), (byte)(time >> 16), (byte)(time >> 8), (byte)(time & 0xFF)}; //todo no matter what it is?

        // report block
        byte receptionReportBlocks[] =  {(byte)(ssrc >> 24), (byte)(ssrc >> 16), (byte)(ssrc >> 8), (byte)(ssrc & 0xFF)}; //todo no matter what it is?
        V_P_RC = (byte)(0x00); //todo no matter what it is?

        // Length is 32 bit words contained in the packet -1
        byte length[] = longToBytes((FIXED_HEADER_SIZE + ss.length
        ...
        rtcpSRPacket = append(rtcpSRPacket, PT);
        rtcpSRPacket = append(rtcpSRPacket, length);
        rtcpSRPacket = append(rtcpSRPacket, ss);
        rtcpSRPacket = append(rtcpSRPacket, NTP_TimeStamp);
        rtcpSRPacket = append(rtcpSRPacket, RTP_TimeStamp);
        rtcpSRPacket = append(rtcpSRPacket, SenderPacketCount);
        rtcpSRPacket = append(rtcpSRPacket, SenderOctetCount);
        rtcpSRPacket = append(rtcpSRPacket, receptionReportBlocks);

        System.out.println(rtcpSRPacket.length);
        System.out.println(this.mcGroup);
        System.out.println(rtcpSession.rtcpMCSock.getPort());

        DatagramPacket packet =new DatagramPacket(rtcpSRPacket,rtcpSRPacket.length,this.mcGroup,rtcpSession.rtcpMCSock.getPort());
